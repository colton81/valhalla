...
struct Polyline3d {
  ...
  Polyline3d() = default;
  Polyline3d(std::vector<std::tuple<double, double, double>> coordinates,
             Precision precision2d,
...
inline std::optional<Error> polyline_decode(std::string_view encoded, Polyline& result) {
  ...
  if (auto error = decode_header(encoded, precision2d_encoded, precision3d_encoded, type3d_encoded)) {
    return *error;
  }
